...
skip_wrap_run_cmd: false                       # 跳过为 run_cmd 添加 PATH 的包装（默认: false）
...
**`skip_wrap_run_cmd`**:
- `false`（默认）：为命令添加 `export PATH=<bin_dir>:$PATH &&` 包装，确保使用运行时环境的可执行文件
- `true`：跳过 PATH 包装，直接使用 `bash -c` 运行命令
...
- `${bin_dir}`: 可选，会被替换为运行时环境的 bin 目录路径
