...
ROCK 1.11 使用持久化状态机管理沙箱创建、停止、重启、归档、恢复、自动过期和删除。生命周期时间参数及归档存储统一配置在 `lifecycle` 下。

完整状态图、配置默认值、转换规则、失败恢复及后端能力差异，见[沙箱生命周期与状态转移控制](lifecycle.md)。
...
- [沙箱生命周期与状态转移控制](lifecycle.md) - 配置并使用完整的沙箱生命周期
